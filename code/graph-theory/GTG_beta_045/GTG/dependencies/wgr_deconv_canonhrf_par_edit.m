function data_deconv = wgr_deconv_canonhrf_par_edit(data,thr,event_lag_max,TR)
...
%event,HRF,adjust_global,PARA
function dat_deconv = wgr_adjust_onset(dat,even_new,event_lag_max,TR,p_m,T,N)
%% global adjust.
kk=1;
...
%     [hrf(:,kk),e3,param(:,kk)] = Fit_Canonical_HRF(dat,TR,design,T,p_m);
    [hrf(:,kk),e3] = Fit_Canonical_HRF(dat,TR,design,T,p_m);
    Cov_E(kk) = cov(e3);
    kk = kk+1;
end
[C,ind] = min(Cov_E);
...
function [hrf, e] = Fit_Canonical_HRF(tc,TR,Run,T,p)
%
