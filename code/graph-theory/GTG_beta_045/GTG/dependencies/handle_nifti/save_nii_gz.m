%%% This file has been edited from the original to handle .gz format
...
%  
%  Usage: save_nii_gz(nii, filename)
%  
...
%
function save_nii_gz(nii, fileprefix)
   ...
   write_nii(nii, filetype, fileprefix, gzip_tf);
   
   return					% save_nii


%-----------------------------------------------------------------------------------
function write_nii(nii, filetype, fileprefix, gzip_tf)
...
%   fwrite(fid, nii.img, precision, skip_bytes);        % error using skip
   ...
   if gzip_tf == 1
       try gzip(sprintf('%s.nii', fileprefix));
           delete(sprintf('%s.nii', fileprefix));
       catch
           eval(['!gzip ',filename]);
       end
   end
