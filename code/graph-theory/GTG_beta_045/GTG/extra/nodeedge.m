function [N,E,w] = nodeedge(W)

% Create separate node and (binary and/or weighted) edge matrices. 
% 
% Author: Jeffrey M. Spielberg (jspielb2@gmail.com)
% Version: 03.07.16
% 
% WARNING: This is a beta version. There no known bugs, but only limited 
% testing has been perfomed. This software comes with no warranty (even the
% implied warranty of merchantability or fitness for a particular purpose).
% Therefore, USE AT YOUR OWN RISK!!!
%
% Copyleft 2014-2016. Software can be modified and redistributed, but 
% modifed, redistributed versions must have the same rights

n = size(W,1);
nonredun = triu(ones(n),1);       % Assumes symmetrical matrix
[N(:,1),N(:,2)] = find(nonredun); % Find node indices
E = W(logical(nonredun));         % Find weighted edge matrix
if nargout > 2                    % Want separate edge & weight matrices?
    w = E;                        % Weight matrix
    E = E ~= 0;                   % Binary edge matrix
end