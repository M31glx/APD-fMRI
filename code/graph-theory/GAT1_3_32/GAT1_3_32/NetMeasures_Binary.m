...
NetMes_Bin{5,1}=assortativity(Graph,0);
...
NetMes_Bin{10,1}=efficiency(Graph);
NetMes_Bin{10,2}='global-efficiency';


NetMes_Bin{11,1}=efficiency(Graph,1);
...
[Mi_L,NetMes_Bin{14,1}]=modularity_louvain_und(Graph);
NetMes_Bin{14,2}='modularity-louvain';


NetMes_Bin{15,1}=charpath(distance_bin(Graph));
...
switch Null
    ...
    case 4
        ...
        if ~isempty(varargin)
            ...
            cor_null_unthr = rand_null_hh (Dat,Iter,Corr);
            ...
            error('provide correlation and data matrix')
