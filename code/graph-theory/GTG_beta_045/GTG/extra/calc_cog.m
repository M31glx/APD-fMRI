...
% Find the center-of-gravity coordinates for a set of ROIs, each of which
% must have a unique label. 
%
% Usage: [cog_orig,obj_labs,cog_trans] = calc_cog(H,transmat)
% 
% Inputs: H        = Either a matrix with ROIs or a structure with the ROI 
%                    matrix labeled as H.img.
%         transmat = (optional) Transformation matrix to some other space.
%                    Could also be contained in the H structure.
% 
% Outputs: cog_orig  = Center of gravity coordinates in matrix space.
%          cog_labs  = Corresponding labels for each ROI.
%          cog_trans = Center of gravity coordinates in transformed space.
% 
% 
...
if isstruct(H)
    if ~exist('transmat','var') && isfield(H,'hdr')
        transmat = [H.hdr.hist.srow_x;H.hdr.hist.srow_y;H.hdr.hist.srow_z];
    end
    H = H.img;
end
