function FDA_Net_Hubs_d(Data1,Data2,Group1,Group2,ROIs,SD)
...
if nargin<1
    ...
    Group1 = ff.mat4GATd.g1;
    Group2 = ff.mat4GATd.g2;
    
    Data1 = spm_select(1,['fda_NetMesReg_' Group1 '.mat'],['Select the "fda_NetMesReg' Group1 '.mat" file ("output from fda for Regional Data")']);
    Data2 = spm_select(1,['fda_NetMesReg_' Group2 '.mat'],['Select the "fda_NetMesReg' Group2 '.mat" file ("output from fda for Regional Data")']);

    load(Data1);load(Data2);
    
    fprintf('%-4s\n',['loading inputs...']);
    ROIs = ff.mat4GATd.roi1;
    %
    GUIinput=1;
end
...
Mdeg1=mean(fda_MDeg_1norm);
SDdeg1=std(fda_MDeg_1norm);
iDeg1=find(fda_MDeg_1norm >= (Mdeg1+SDdeg1*SD));
HubInd1=iDeg1;   
...
Mbetw1=mean(fda_MNodeBetw_1norm);
SDbetw1=std(fda_MNodeBetw_1norm);
iBetw1=find(fda_MNodeBetw_1norm >= (Mbetw1+SDbetw1*SD));
HubInd1=iBetw1;   
...
Mdeg2=mean(fda_MDeg_2norm);
SDdeg2=std(fda_MDeg_2norm);
iDeg2=find(fda_MDeg_2norm >= (Mdeg2+SDdeg2*SD));
HubInd2=iDeg2;   
...
Mbetw2=mean(fda_MNodeBetw_2norm);
SDbetw2=std(fda_MNodeBetw_2norm);
iBetw2=find(fda_MNodeBetw_2norm >= (Mbetw2+SDbetw2*SD));
HubInd2=iBetw2;   
